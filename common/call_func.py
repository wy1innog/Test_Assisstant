...
class Call_func(object):
    ...
    def getCall_config(self) -> tuple:
        ...
        return call_config
        # print(call_config)
        # number = call_config['call_number']
        # hold = call_config['call_hold']
        # timeout = call_config['call_timeout']
        # interval = call_config['call_interval']
        # return number, hold, timeout
    ...
    def cp_calling_to_answer(self):
        ...
        while True:
            ...
            elif endtime-starttime == timeout:
                print("通话超时，主动挂断")
                nf.exec_cmd('AT+CHUP')
                return nf.process
            else:
    ...
    def cp_caller_hangs_up(self) -> list:
        ...
        nf = Normal_func()
        nf.process.clear()
        number, hold, timeout = Call_func.call_config()
        nf.exec_cmd('ATD%s;' % number)
        ...
        while True:
            ...
            if nf.process[-1] == "拨号":
                endtime = time.time()
                print("dial process: %s" % nf.process)
    ...
    def cp_call_no_answer(self) -> list:
        ...
        while True:
            ...
            elif nf.process[-1] != "通话结束" and endtime-starttime == timeout:
                print("振铃超时，未挂断，已超过设定值")
...
if __name__ == '__main__':
    config = Call_func.getCall_config()
    print(config)