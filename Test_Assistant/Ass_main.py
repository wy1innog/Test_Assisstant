...
class Ass(QMainWindow, Ui_MainWindow, QComboBox):
        ...
        if at_cmd1 != "刷新":
            if self.ser.is_open:
                try:
                    ...
                    logger.info("Send choose at  ./:%s" % at_cmd1)
    ...
    def _calling_to_answer(self, times, number):
        ...
        self.CP_recv_textBrowser.append("测试项：主叫接听 测试次数:%s" % times)
        self.recv_flag = False
        current_process = ['拨号', '对端响铃', '对端接听', '通话结束']
        ...
        for i in range(int(times)):
            ...
            if self.test_flag:
                self.process.clear()
                time.sleep(2)
                self.CP_recv_textBrowser.append("主叫接听 >> 第%s次, 共%s次" % (i + 1, times))
                self._exec_cmd('ATD%s;' % number)

                logger.info("主叫接听 >> 第%s次, 共%s次," % (i + 1, times))
                time.sleep(2)
                ...
                break
        self.CP_recv_textBrowser.append("测试项：主叫接听 测试次数:%d, pass:%d, fail:%d\r\n" %
                                        (i, pass_calling_to_answer, fail_calling_to_answer))
        self.recv_to_bottom()

        logger.info("测试项：主叫接听 测试次数:%d, pass:%d, fail:%d\r\n" %
                    (i, pass_calling_to_answer, fail_calling_to_answer))
    ...
    def _caller_hangs_up(self, times, number):
        ...
        pass_hangs_up = 0
        fail_hangs_up = 0
        self.CP_recv_textBrowser.append("测试项：主叫挂断 测试次数:%s" % times)
        self.recv_flag = False
        current_process = ['拨号', '对端响铃', '通话结束']
        ...
        for i in range(int(times)):
            ...
            if self.test_flag:
                self.process.clear()
                time.sleep(2)
                self.CP_recv_textBrowser.append("主叫挂断 >> 第%s次, 共%s次," % (i + 1, times))
                self._exec_cmd('ATD%s;' % number)
                logger.info("主叫挂断 >> 第%s次, 共%s次," % (i + 1, times))
                time.sleep(2)
                ...
                break
        self.CP_recv_textBrowser.append("测试项：主叫挂断 测试次数:%d, pass:%d, fail:%d\r\n" %
                                        (i, pass_hangs_up, fail_hangs_up))
        self.recv_to_bottom()
        logger.info("测试项：主叫接听 测试次数:%d, pass:%d, fail:%d\r\n" %
                    (i, pass_hangs_up, fail_hangs_up))
    ...
    def _calling_reject(self, times, number):
        ...
        self.recv_flag = False
        current_process = ['拨号', '对端响铃', 'busy, 通话结束']
        ...
        self.CP_recv_textBrowser.append("测试项：主叫拒接 测试次数:%s, pass:%d, fail:%d\r\n" %
                                        (i, pass_calling_reject, fail_calling_reject))
        self.recv_to_bottom()
        logger.info("测试项：主叫拒接 测试次数:%s, pass:%d, fail:%d\r\n" %
                    (i, pass_calling_reject, fail_calling_reject))
    ...
    def _no_caller_answer(self, times, number):
        ...
        self.recv_flag = False
        current_process = ['拨号', '对端响铃', '对端无应答, 通话结束']
        ...
        self.CP_recv_textBrowser.append("测试项：主叫未接 测试次数:%s, pass:%d, fail:%d\r\n" %
                                        (i, pass_no_caller_answer, fail_no_caller_answer))
        self.recv_to_bottom()
